...
import { CREATE_SET, READ_ALL_USER_EXERCISES } from "../../schema";
import { useStringInput } from "../inputs";
import MutationForm from "./MutationForm";

const CreateSetForm = ({ date, exercise }: any) => {
  const { ...weight } = useStringInput("Weight");
  const { ...systemOfMeasurement } = useStringInput("Sys Of Meas");
  const { ...reps } = useStringInput("Reps");
  const { ...rpe } = useStringInput("RPE");
  const inputs = [weight, systemOfMeasurement, reps, rpe];
  const createSetInput = {
    warmUp: false,
    weight: Number(weight.value),
    systemOfMeasurement: systemOfMeasurement.value,
